...
function parseData(data){
  ...
  var dimensions = data.fields.filter(function(d) {return d.concept === "DIMENSION"});
  var dimensionId1 = dimensions[0].id;
  var dimensionId2 = dimensions[1].id;

  // assuming only one metric
  var metricId = data.fields.find(function(d) {return d.concept === "METRIC"}).id;

  var dimNodes1 = data.rows.map(function(row) {return row[dimensionId1];})
  var dimNodes2 = data.rows.map(function(row) {return row[dimensionId2];})
  
  var uniqueNodes = Array.from(new Set(dimNodes1.concat(dimNodes2)));

  var nodes = uniqueNodes.map(function(d){return {"id": d};});

  // this assumes uniqueness of dim x dim in rows
  var links = data.rows.map(function(row){
    return {"source": uniqueNodes.indexOf(row[dimensionId1]),
  ...
  if (style.show_labels){
    // add the "left" side labels
     svg.append("g")
      ...
      .attr("x", function(d) {return style.left_offset? d.x0 + +style.left_offset : d.x0 + margin.right/2;})
    ...
    // add the "right" side labels
     svg.append("g")
      ...
      .append("text")
      .filter(function(d) {return d.x0 > width/2})
      .attr("x", function(d) {return style.right_offset? d.x0 - +style.right_offset : d.x0 - margin.right/2;})
      .attr("y", function(d) {return (d.y0 + d.y1)/2;})
