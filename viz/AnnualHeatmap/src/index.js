...
export const LOCAL = false;
  ...
  function getTidyData(tableData) {
    ...
    return tableData.tables.DEFAULT.rows.map(datarow => mapToColumns_(datarow, headers));
  }
  ...
  function mapToColumns_(datarow, headers) {
    ...
    datarow.forEach((fieldValue, i) => {
      rowDataObj[headers[i].configId] = fieldValue;
    });
    return rowDataObj;
  }
  ...
  function getValue(config) {
    ...
    let value =
        config.hasOwnProperty('value') ? config.value : config.defaultValue;
    if (value == null) {
  ...
  class dsField {
    /**
     * @param {!Array<!RowHeader>} headers
     * @param {string} configId config Id
     * @return {string} name of the field
     * @memberof dsField
     */
    constructor(headers, configId) {
      /**
       * @private
       * @const {string}
       */
       ...
       * @const {string}
       */
      this.name_ = this.getFieldName_(headers);
      
    }

    /**
     * @return {string} returns config Id
     */
    get configId() {
     ...
     * @memberof dsField
     */
    getFieldName_(headers) {
        for (let index = 0; index < headers.length; ++index) {
          const field = headers[index];
          if (field.configId === this.configId_) {
            return field.name;
          }
        }
      return '';
  ...
  function createContainer(containerId) {
    if (!document.querySelector('div')) {
      ...
      newdiv.setAttribute(
          'style', 'padding-right:0');  // vega embed adds 32px padding-right
  ...
  function getVegaTheme(theme, overrideValues = {}) {
    const primaryColor = overrideValues.primaryColor || theme.themeSeriesColor[0].color;
    ...
    const themeColors =
        theme.themeSeriesColor.map((themeValue) => themeValue.color);
  ...
  function drawViz(request) {
    ...
    const dateDimensionField = new dsField(headers, 'Date');
    const tempDimensionField = new dsField(headers, 'Temperature');
    const aggregationValue = getValue(style.aggregation);
    ...
    let barLayer = {
            ...
            'encoding': {
              'x': {'field': dateDimensionField.configId,'timeUnit':'date', 'type': 'ordinal','title':'Day'},
              'y': {'field':  dateDimensionField.configId,'timeUnit':'month', 'type':'ordinal','title':'Month'},
            }
    };
    
    barLayer.encoding.color = {
            'field': tempDimensionField.configId,
    ...
    if (byCategoryColor == true) {
      barLayer.encoding.color.scale= {'range': [minScale,maxScale]};
    } else {
      barLayer.encoding.color.scale= {'range': [dsThemeColors[0],dsThemeColors[1]]};
    }
    spec.layer.push(barLayer);
