...
function drawViz(data){
  ...
  let style = data.style.pageStyle.value;
    ...
    function loadData(){ 
      ...
      $.each(rowData,function(j,row){
        ...
        for(var k in dimensionData){
          let finalData = $('<div class="field">');
          let fieldElement = '<span class="'+fieldArray[k]+'"'+'>'+fieldArray[k]+" - "+'</div>';
          let data = '<span class="'+dimensionData[k]+'"'+'>'+dimensionData[k]+'</div>';
          finalData.append(fieldElement+data);
          rowDiv.append(finalData);
        }
        mainDiv.append(rowDiv);
      });
      $("body").addClass("container").append(mainDiv);
    }
