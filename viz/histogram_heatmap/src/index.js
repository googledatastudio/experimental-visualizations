...
export const LOCAL = false;
  ...
  function getTidyData(tableData) {
    ...
    return tableData.tables.DEFAULT.rows.map(
        datarow => mapToColumns_(datarow, headers));
  }
  ...
  function drawViz(request) {
    ...
    let spec = {
      ...
      'transform': [
        {'filter':{'and':[{'field':dimensionField[0]["id"],'valid':true},{'field':dimensionField[1]["id"],'valid':true}]}}
      ],
      'layer':[],
      'config':{
        'view':{
    ...
    if (hideXAxis == true) {
      histogramLayer.encoding.x.axis= {'domain': false,'labels':false,'ticks':false,'title':null};
    } 
    if(hideYAxis == true){
      histogramLayer.encoding.y.axis= {'domain': false,'labels':false,'ticks':false,'title':null};
    }
    ...
    if (byCategoryColor == true) {
      ...
      histogramLayer.encoding.color.scale= {'range': [dsThemeColors[0],dsThemeColors[1]]};
    }
