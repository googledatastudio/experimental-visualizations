...
export function drawContent() {
  ...
  function changeRoot(p) {
    const historyStart = aHistory.length;
    const lastHistoryChild = aHistory[historyStart - 1];
    /**
    ...
    /**
     * To DO
     * Re-adjust colors on drill-down
     */
    ...
    const t = ctx.g.transition().duration(750);
    ...
    let level = p.depth;
    let node = p;
    if (aHistory.length < historyStart) {
      level = lastHistoryChild.depth;
      node = lastHistoryChild;
    }
    ctx.buildLegend(node, level);
  }
}
