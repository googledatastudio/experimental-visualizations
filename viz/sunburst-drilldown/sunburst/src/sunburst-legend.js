...
export function buildLegend(root, level) {
    ...
    if (this.legend) {
        ...
        const legUnit = 20;
        const maxLegend = Math.floor(
            (this.dimensions.boundedRadius * 2) / legUnit
        );
        ...
        const breadcrumb = this.wrapper
            ...
            .style(
                "transform",
                (d, i) =>
                    `translate(${this.dimensions.boundedRadius + 50}px, ${i *
                        legUnit -
                        legStart -
        ...
        const legendBar = this.wrapper
            ...
            .data(
                root
                    .descendants()
                    .filter(
                        (d, i) => i < maxLegend && d.data.depth == level + 1
                    )
            )
            .enter()
            .append("g")
        ...
        legendBar
            ...
            .attr(
                "fill",
                level == 0 ? d => this.colorScale(d.data.id) : "black"
            )
