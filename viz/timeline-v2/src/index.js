...
const d3 = Object.assign({}, require("d3"), require('d3-scale'), require('d3-transition'), require('d3-axis'));
...
const drawViz = (data) => {
  ...
  let style = {
      buttonColor: data.style.buttonColor.value !== undefined ? data.style.buttonColor.value.color : data.style.buttonColor.defaultValue,
      axisColor: data.style.axisColor.value !== undefined ? data.style.axisColor.value.color : data.style.axisColor.defaultValue,
      vizBackground: data.style.vizBackground.value !== undefined ? data.style.vizBackground.value.color : data.style.vizBackground.defaultValue,
      fontFamily: data.style.font.value !== undefined? data.style.font.value : data.style.font.defaultValue
  };
  const margin = { top: 20, bottom: 20, left: 20, right: 20 };
    ...
    // draw buttons
    const years = Array.from(new Set(vizData.map((d) => d.year))).sort().reverse();
    ...
    function onMouseover(grouped) {
        ...
        const sortedBullets = grouped.values.sort((a, b)=>a.category > b.category);
    ...
    function onMouseout(d) {
        d3.select(this)
            .attr('opacity', 0);
        textArea.selectAll('text, circle')
            .remove();
    }
    ...
    function drawTimeline(parsedData, timelineSvg) {
        ...
        const categoryNested = d3.nest()
            .key((d) => d.dateStr)
            .key((d) => d.category)
            .entries(parsedData);

        const byDateCategory = categoryNested.reduce((acc, d) => { return acc.concat(d.values) }, []).map((d) => {
            return {
        ...
        vizCanvas
            ...
            .attr('transform', `translate(${timelineMargin.left}, ${vizHeight - timelineMargin.bottom})`);
